#ifndef RMAPUTILITIES_HH_
#define RMAPUTILITIES_HH_

#include "RMAPDestination.hh"

#include <vector>
using namespace std;

/** An internal class used in the RMAPSocket class.
 * This class holds utility methods which is often needed
 * by executing RMAP operation such as packet creation and 
 * interpretation.
 */ 

class RMAPUtilities {
public:
	/** Calculates a CRC code. The version of the CRC code
	 * is designated in RMAPDestination argument.
	 */ 
	static unsigned char calcCRC(RMAPDestination& destination, vector<unsigned char>& array);
	
};


//-----------------------------------------------------------
// CRC Table from RMAP spec draft F
static const unsigned char RMAP_CRCTable_DraftF[] = {  };

// CRC Table from RMAP spec draft E
static const unsigned char RMAP_CRCTable_DraftE[] = {  };
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^


#endif /*RMAPUTILITIES_HH_*/
