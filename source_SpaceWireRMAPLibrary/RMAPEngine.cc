...
void RMAPEngine::start() {
	...
	suspended = false;
	stopped = false;
	replyprocessthread = new RMAPEngine_ReplyProcessThread(getSpaceWireIF(), this, &tidmapmutex, &replymapmutex,
			&replyloopcondition, &tidmap, &replymap, &destinationmap, &destinationmapmutex, &receivedpacketqueue,
			&receivedpacketqueuemutex, &coutmutex, &startstopmutex_send);
	replyprocessthread->start();
	Condition c;
	c.wait(100);
	requestprocessthread = new RMAPEngine_RequestProcessThread(getSpaceWireIF(), this, &tidmapmutex, &requestmapmutex,
			&requestloopcondition, &tidmap, &requestmap, &destinationmap, &destinationmapmutex, &sentpacketqueue,
			&sentpacketqueuemutex, &coutmutex, &startstopmutex_receive);
...
void RMAPEngine::stop() {
	...
	if (stopped == false) {
		...
		for (i = 0; i < 20; i++) {
			...
			if (true == stopper_request.finished)
				break;
			c.wait(100);
		}
		if (20 == i) {
		...
		for (i = 0; i < 20; i++) {
			...
			if (true == stopper_reply.finished)
				break;
			c.wait(100);
		}
		if (20 == i) {
...
void RMAPEngine::requestCommand(RMAPSocket* socket, RMAPPacket* commandpacket) throw (RMAPException) {
	if (suspended == true || stopped == true) {
...
RMAPEngine_RequestProcessThread::RMAPEngine_RequestProcessThread(SpaceWireIF* spwif, RMAPEngine* engine,
		Mutex* newtidmapmutex, Mutex* mutex, Condition* condition, map<unsigned int, unsigned int>* newtidmap, map<
				unsigned int, RMAPPacket*>* newrequestmap, map<unsigned int, RMAPDestination*>* newdestinationmap,
		Mutex* newdestinationmapmutex, queue<vector<unsigned char>*>* newsentpacketqueue,
		Mutex* newsentpacketqueuemutex, Mutex* newcoutmutex, Mutex* startstopmutex_send) :
	Thread(), spacewireif(spwif), rmapengine(engine), tidmapmutex(newtidmapmutex), requestmapmutex(mutex),
			requestloopcondition(condition), tidmap(newtidmap), requestmap(newrequestmap), destinationmap(
					newdestinationmap), destinationmapmutex(newdestinationmapmutex),
...
RMAPEngine_ReplyProcessThread::RMAPEngine_ReplyProcessThread(SpaceWireIF* spwif, RMAPEngine* engine,
		Mutex* newtidmapmutex, Mutex* mutex, Condition* condition, map<unsigned int, unsigned int>* newtidmap, map<
				unsigned int, RMAPPacket*>* newreplymap, map<unsigned int, RMAPDestination*>* newdestinationmap,
		Mutex* newdestinationmapmutex, queue<vector<unsigned char>*>* newreceivedpacketqueue,
		Mutex* newreceivedpacketqueuemutex, Mutex* newcoutmutex, Mutex* startstopmutex_receive) :
	Thread(), spacewireif(spwif), rmapengine(engine), tidmapmutex(newtidmapmutex), replymapmutex(mutex),
			replyloopcondition(condition), tidmap(newtidmap), replymap(newreplymap), destinationmap(newdestinationmap),
			destinationmapmutex(newdestinationmapmutex), receivedpacketqueue(newreceivedpacketqueue),
			receivedpacketqueuemutex(newreceivedpacketqueuemutex), coutmutex(newcoutmutex), startstopmutex_receive(
					startstopmutex_receive) {
}
...
#endif
			if (e.getStatus() == SpaceWireException::Disconnected) {
				rmapengine->stop();
				return;
			}
			if (e.getStatus() == SpaceWireException::Timeout && rmapengine->isStopped()) {
				return;
			}
			rmapengine->handleSpaceWireException(e);
			goto runloop;
